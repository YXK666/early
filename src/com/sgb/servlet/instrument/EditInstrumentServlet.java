 ...
 *
 */
@WebServlet("/editInstrumentServlet.do")
public class EditInstrumentServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		itm.setId(id);
		System.out.println("初始查询的对象"+id);
		String instrumentName = req.getParameter("instrumentName");
		if(instrumentName!=null&!"".equals(instrumentName)) {itm.setInstrumentName(instrumentName);}
		
		String instrumentNumber = req.getParameter("instrumentNumber");
		if(instrumentNumber!=null&!"".equals(instrumentNumber)) {itm.setInstrumentNumber(instrumentNumber);}
		...
		System.out.println("修改PM前端控制器AddInstrumentServlet你前端的初始对象1："+itm.toString());
		boolean b = false;
		b = InstrumentDao.updateInstrument(itm, id);//修为一个初始查询的对象
		System.out.println("初始查询的对象添加"+b);
		itm.setId(0);
		itm.setStatus(1);//添加一个待查询的对象状态为1，		
		if(planTime!=null&&!"".equals(planTime)) {itm.setPlanTime(planTime);}//一个计划时间		
		itm.setFinishTime(null);//将完成时间设置为空
		InstrumentDao.addInstrument(itm);
		System.out.println("添加PM前端控制器AddInstrumentServlet用于检查：你前端的对象2："+itm.toString());
		if(b) {
