 ...
 *
 */
@WebServlet("/listCheckManServlet.do")
public class ListCheckManServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		//传递参数
		int totalRows = 0;
		List<CheckMan> result = null;
		try {
			result = CheckManDao.getAllCheckMan(pageNum, pageSize);
			totalRows = CheckManDao.getTotalRows();
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
		JSONArray jsonArray = new JSONArray();
		for(CheckMan d : result) {
			jsonArray.add(d);
		}
